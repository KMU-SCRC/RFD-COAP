...
unsigned char id[50] = "AA";
unsigned char topic[50] = "PENGUIN/1";
unsigned char filename[100] = "2MB_PENGUIN.csv";
unsigned char filename_origin[100] = "2MB_PENGUIN.csv";
unsigned char* filename_cut;
#ifdef _WIN32
unsigned char fileroot[200] = "D:\\data\\sampledata.csv";
unsigned char logroot[200] = "D:\\data\\RFD_COAP_SYSTEM_LOG.TXT";
unsigned char statroot[200] = "D:\\data\\RFD_NETWORK_STAT.TXT";
unsigned char configroot[200] = "D:\\RFDCONFIG\\RFDCONFIG.INI";
#else
unsigned char fileroot[200] = "/home/pi/Projects/data/sampledata.csv";
unsigned char logroot[200] = "/home/pi/Projects/data/RFD_COAP_SYSTEM_LOG.TXT";
unsigned char statroot[200] = "/home/pi/Projects/data/RFD_NETWORK_STAT.TXT";
unsigned char configroot[200] = "/home/pi/Projects/RFDCONFIG/RFDCONFIG.INI";
#endif
FILE* logfile = NULL;
unsigned char urihost[100] = "coap://kmuscrc-ffd/";
unsigned char urihost_input[80] = "kmuscrc-ffd";

unsigned int start_switch = 0;
unsigned int stop_switch = 0;
unsigned int restart_switch = 0;
unsigned int retry_switch = 0;
//unsigned int done_switch = 0;
//unsigned int continue_switch = 0;
unsigned int delete_switch = 0;
unsigned int exit_switch = 0;
unsigned int stat_switch = 0;
unsigned int infinity_switch = 0;
unsigned int infinity_count = 1;
...
unsigned char fileBuffer[1000];
unsigned int number = 1;
unsigned int toggle = 0;
unsigned int maxnum = 1;
unsigned int difnum = 0;
unsigned int pernum = 1;
unsigned char url[200] = "coap://kmuscrc-ffd/file?";
...
static coap_optlist_t* optlistf = NULL;
/* Request URI.
 * TODO: associate the resources with transaction id and make it expireable */
static coap_uri_t uri;
static coap_uri_t urif;
...
message_handler2(coap_session_t* session COAP_UNUSED,
    ...
    if (coap_get_data_large(received, &len, &databuf, &offset, &total)) {
        snprintf((char*)str, len + 1, "%s", databuf);
        cut = strtok(str, ":");
        if (strcmp(cut, "OK") == 0) {
            if (!start_switch) {
...
static int
cmdline_urif(char* arg, int create_uri_opts) {
    unsigned char portbuf[2];
#define BUFSIZE 100
    unsigned char _buf[BUFSIZE];
    unsigned char* buf = _buf;
    ...
    if (!proxy_scheme_option && proxy.host.length) {
        ...
        if (coap_split_uri((unsigned char*)arg, strlen(arg), &urif) < 0) {
            coap_log(LOG_ERR, "invalid CoAP URI\n");
            return -1;
        }

        if (urif.scheme == COAP_URI_SCHEME_COAPS && !reliable && !coap_dtls_is_supported()) {
            coap_log(LOG_EMERG,
                "coaps URI scheme not supported in this version of libcoap\n");
            return -1;
        }

        if ((urif.scheme == COAP_URI_SCHEME_COAPS_TCP || (urif.scheme == COAP_URI_SCHEME_COAPS && reliable)) && !coap_tls_is_supported()) {
            coap_log(LOG_EMERG,
                "coaps+tcp URI scheme not supported in this version of libcoap\n");
            return -1;
        }

        if (urif.scheme == COAP_URI_SCHEME_COAP_TCP && !coap_tcp_is_supported()) {
            /* coaps+tcp caught above */
            coap_log(LOG_EMERG,
                "coap+tcp URI scheme not supported in this version of libcoap\n");
            return -1;
        ...
        if (urif.port != get_default_port(&urif) && create_uri_opts) {
            coap_insert_optlist(&optlistf,
                coap_new_optlist(COAP_OPTION_URI_PORT,
                    coap_encode_var_safe(portbuf, sizeof(portbuf),
                        (urif.port & 0xffff)),
        ...
        if (urif.path.length) {
            buflen = BUFSIZE;
            if (urif.path.length > BUFSIZE)
                coap_log(LOG_WARNING, "URI path will be truncated (max buffer %d)\n", BUFSIZE);
            res = coap_split_path(urif.path.s, urif.path.length, buf, &buflen);
        ...
        if (urif.query.length) {
            buflen = BUFSIZE;
            buf = _buf;
            res = coap_split_query(urif.query.s, urif.query.length, buf, &buflen);
...
open_session(
    ...
    if (proto == COAP_PROTO_DTLS || proto == COAP_PROTO_TLS) {
        ...
        if (root_ca_file || ca_file || cert_file) {
            /* Setup PKI session */
            coap_dtls_pki_t* dtls_pki = setup_pki(ctx, COAP_DTLS_ROLE_CLIENT, uri.host.s);
...
main(int argc, char **argv) {
  ...
  coap_context_t* ctxc = NULL;
  coap_context_t* ctxf = NULL;
  coap_session_t* session = NULL;
  coap_session_t* sessionf = NULL;
  coap_address_t dst;
  coap_address_t dstf;
  ...
  static coap_str_const_t serverf;
  uint16_t port = COAP_DEFAULT_PORT;
  uint16_t portf = COAP_DEFAULT_PORT;
      ...
      if (fstat(fileno(file), &statbuf) < 0) {
          ...
          if (len < 0) {
              ...
              if ((ID_CUT != NULL) && (TOPIC_CUT != NULL) && (FILENAME_CUT != NULL) && (FILEROOT_CUT != NULL) && (LOGROOT_CUT != NULL) && (URIHOST_CUT != NULL) && ((ID_CUT = strtok(ID_CUT, "=")) != NULL) && (strcmp(ID_CUT, "ID") == 0) && ((ID_CUT = strtok(NULL, "=")) != NULL) && ((TOPIC_CUT = strtok(TOPIC_CUT, "=")) != NULL) && (strcmp(TOPIC_CUT, "TOPIC") == 0) && ((TOPIC_CUT = strtok(NULL, "=")) != NULL) && ((FILENAME_CUT = strtok(FILENAME_CUT, "=")) != NULL) && (strcmp(FILENAME_CUT, "FILENAME") == 0) && ((FILENAME_CUT = strtok(NULL, "=")) != NULL) && ((FILEROOT_CUT = strtok(FILEROOT_CUT, "=")) != NULL) && (strcmp(FILEROOT_CUT, "FILEROOT") == 0) && ((FILEROOT_CUT = strtok(NULL, "=")) != NULL) && ((LOGROOT_CUT = strtok(LOGROOT_CUT, "=")) != NULL) && (strcmp(LOGROOT_CUT, "LOGROOT") == 0) && ((LOGROOT_CUT = strtok(NULL, "=")) != NULL) && ((URIHOST_CUT = strtok(URIHOST_CUT, "=")) != NULL) && (strcmp(URIHOST_CUT, "URIHOST") == 0) && ((URIHOST_CUT = strtok(NULL, "=")) != NULL)) {
                  ...
                  sprintf(urihost, "coap://%s/", URIHOST_CUT);
                  sprintf(urihost_input, URIHOST_CUT);
              }
              else {
                  rewrite_switch = 1;
              }
  ...
  while ((opt = getopt(argc, argv, "a:D:Ef:F:I:tT:U:c:d:eg:G:h:i:j:J:k:l:mnp:rs:u:v:A:C:L:M:NP:R:S:X:")) != -1) {
    switch (opt) {
    case 'a':
    ...
    case 'u':
      ...
      break;
    case 'U':
      sprintf(urihost, "coap://%s/", optarg);
      sprintf(urihost_input, optarg);
      rewrite_switch = 1;
      break;
    case 'v' :
  ...
  if (rewrite_switch) {
      file = fopen(configroot, "w");
      sprintf(fileBuffer, "ID=%s\nTOPIC=%s\nFILENAME=%s\nFILEROOT=%s\nLOGROOT=%s\nURIHOST=%s\n\n\n\n ID(RFD 기기의 고유값)\n TOPIC(RFD가 보내고자 하는 파일의 주제)\n FILENAME(FFD에 저장될 파일명)\n FILEROOT(RFD가 보내고자 하는 파일 전체 경로)\n LOGROOT(RFD가 로그 파일을 저장할 전체 경로)\n URIHOST(FFD의 HOST주소)\n 순으로 변수명뒤에 띄어쓰기 없이 =과 텍스트를 붙임으로서 설정할 수 있으며 줄바꿈으로 값을 구별합니다.\n", id, topic, filename, fileroot, logroot, urihost_input);
      fputs(fileBuffer, file);
      fflush(file);
      fclose(file);
  }
  ...
  sprintf(url, "%scommand?READY:%s:%s:%s:%d", urihost, id, topic, filename, maxnum);
  cmdline_uri(url, 1);//주소 대입 _scrc
  server = uri.host;
  port = uri.port;
  scheme = uri.scheme;
  res = resolve_address(&server, &dst.addr.sa);//서버값을 목적지에 넣고 사이즈를 반환 _scrc
  ctxc = coap_new_context(NULL);//클라이언트용 새 컨택스트 ctxc _scrc
  coap_context_set_keepalive(ctxc, ping_seconds);//킵얼라이브 타이머 설정 _scrc
  coap_context_set_block_mode(ctxc, block_mode);//블록모드 사용 여부 _scrc
  if (csm_max_message_size)//TCP연결시 최대 메시지 사이즈 _scrc
      coap_context_set_csm_max_message_size(ctxc, csm_max_message_size);
  coap_register_response_handler(ctxc, message_handler2);//응답을 수신할 때마다 호출되는 핸들러 _scrc
  coap_register_event_handler(ctxc, event_handler);//이벤트 핸들러 _scrc
  coap_register_nack_handler(ctxc, nack_handler);//메시지를 전송할 수 없을때 호출되는 핸들러 _scrc
  dst.size = res;
  dst.addr.sin.sin_port = htons(port);
  session = coap_new_client_session(ctxc, NULL, &dst, COAP_PROTO_UDP);//세션생성 _scrc

  coap_insert_optlist(&optlist,//옵션리스트 _scrc
      coap_new_optlist(COAP_OPTION_URI_HOST,//옵션 _scrc
          uri.host.length,
          uri.host.s));

  method = cmdline_method("get");
  //==========================================================================
  sprintf(url, "%sfile?%s:1", urihost, id);
  cmdline_urif(url, 1);//주소 대입 _scrc
  serverf = urif.host;
  portf = urif.port;
  schemef = urif.scheme;
  resf = resolve_address(&serverf, &dstf.addr.sa);//서버값을 목적지에 넣고 사이즈를 반환 _scrc
  ctxf = coap_new_context(NULL);//클라이언트용 새 컨택스트 ctxc _scrc
  coap_context_set_keepalive(ctxf, ping_seconds);//킵얼라이브 타이머 설정 _scrc
  coap_context_set_block_mode(ctxf, block_mode);//블록모드 사용 여부 _scrc
  if (csm_max_message_size)//TCP연결시 최대 메시지 사이즈 _scrc
      coap_context_set_csm_max_message_size(ctxf, csm_max_message_size);
  coap_register_response_handler(ctxf, message_handler2);//응답을 수신할 때마다 호출되는 핸들러 _scrc
  coap_register_event_handler(ctxf, event_handler);//이벤트 핸들러 _scrc
  coap_register_nack_handler(ctxf, nack_handler);//메시지를 전송할 수 없을때 호출되는 핸들러 _scrc
  dstf.size = resf;
  dstf.addr.sin.sin_port = htons(portf);
  sessionf = coap_new_client_session(ctxf, NULL, &dstf, COAP_PROTO_UDP);//세션생성 _scrc

  /* set block option if requested at commandline */
  if (flags & FLAGS_BLOCK)
      set_blocksize();

  methodf = cmdline_method("put");
  ...
  if (exit_switch) {
      coap_delete_optlist(optlist);
      optlist = NULL;
      sprintf(url, "%scommand?EXIT:", urihost);
      cmdline_uri(url, 1);
      coap_insert_optlist(&optlist,//옵션리스트 _scrc
          coap_new_optlist(COAP_OPTION_URI_HOST,//옵션 _scrc
              uri.host.length,
      ...
      if (file == NULL) {
          ...
          if (!(fgets(fileBuffer, sizeof(fileBuffer), file) == NULL) && ((QUALITY_CUT = strtok(fileBuffer, "=")) != NULL) && ((QUALITY_CUT = strtok(NULL, "=")) != NULL) && ((SIGNAL_LEVEL_CUT = strtok(NULL, "=")) != NULL) && ((QUALITY_CUT = strtok(QUALITY_CUT, " ")) != NULL) && ((SIGNAL_LEVEL_CUT = strtok(SIGNAL_LEVEL_CUT, " ")) != NULL)) {
              coap_delete_optlist(optlist);
              optlist = NULL;
              sprintf(url, "%scommand?READYANDSTAT:%s:%s:%s:%d:%s:%s", urihost, id, topic, filename, maxnum, QUALITY_CUT, SIGNAL_LEVEL_CUT);
              cmdline_uri(url, 1);
              coap_insert_optlist(&optlist,//옵션리스트 _scrc
                  coap_new_optlist(COAP_OPTION_URI_HOST,//옵션 _scrc
                      uri.host.length,
                      uri.host.s));
              sprintf(url, "ID : %s, TOPIC : %s, FILENAME : %s, MAXNUM : %d, NETWORK QUALITY : %s, NETWORK SIGNAL LEVEL : %s dBm\n", id, topic, filename, maxnum, QUALITY_CUT, SIGNAL_LEVEL_CUT);
  ...
  while (!quit) {
      while (exit_switch && !quit) {
          ...
          pdu = coap_new_request(ctxc, session, method, &optlist, NULL, 0);
          coap_send(session, pdu);
          result = coap_io_process(ctxc, timeout_ms);
      }
	  ...
	  while (!delete_switch && !retry_switch && !quit) {
          ...
          while (!retry_switch && !quit) {
			  ...
			  if (number >= toggle) {
				  cmdline_input(fileBuffer, &payload);
				  if (payload.length) {
				  ...
				  coap_delete_optlist(optlistf);
				  optlistf = NULL;
				  sprintf(url, "%sfile?%s:%d", urihost, id, number);
				  cmdline_urif(url, 1);
				  coap_insert_optlist(&optlistf,//옵션리스트 _scrc
					  coap_new_optlist(COAP_OPTION_URI_HOST,//옵션 _scrc
						  urif.host.length,
						  urif.host.s));
				  pduf = coap_new_request(ctxf, sessionf, methodf, &optlistf, data, data_len);
				  coap_send(sessionf, pduf);
				  result = coap_io_process(ctxf, timeout_ms);
				  if (!timer) {
                      #ifdef _WIN32
					  startc = clock();
                      #else
					  ...
					  logfile = fopen(logroot, "a");
					  sprintf(url, "START : %s", asctime(pinfo));
					  printf(url);
					  fputs(url, logfile);
					  fflush(logfile);
					  fclose(logfile);
			  ...
			  if (stop_switch) {
				  coap_delete_optlist(optlist);
				  optlist = NULL;
				  sprintf(url, "%scommand?OK:%s", urihost, id);
				  cmdline_uri(url, 1);
				  coap_insert_optlist(&optlist,//옵션리스트 _scrc
					  coap_new_optlist(COAP_OPTION_URI_HOST,//옵션 _scrc
						  uri.host.length,
		  ...
		  coap_delete_optlist(optlist);
		  optlist = NULL;
		  sprintf(url, "%scommand?DONE:%s:%d", urihost, id, number);
		  cmdline_uri(url, 1);
		  coap_insert_optlist(&optlist,//옵션리스트 _scrc
			  coap_new_optlist(COAP_OPTION_URI_HOST,//옵션 _scrc
				  uri.host.length,
      ...
      if (delete_switch) {
          coap_delete_optlist(optlist);
          optlist = NULL;
          sprintf(url, "%scommand?EMPTY:%s", urihost, id);
          cmdline_uri(url, 1);
          coap_insert_optlist(&optlist,//옵션리스트 _scrc
              coap_new_optlist(COAP_OPTION_URI_HOST,//옵션 _scrc
                  uri.host.length,
  ...
  coap_free_context(ctxc);
  coap_delete_optlist(optlistf);
  coap_session_release(sessionf);
  coap_free_context(ctxf);
  close_output();
